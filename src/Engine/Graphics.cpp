...
CR::Gfx::Renderable *CR::Gfx::Draw::Texture(CR::Gfx::Texture &tex, const CR::Vec2<float> &pos, const CR::Vec2<int> &size, const CR::Vec2<float> &origin, float angle){
    CR::Gfx::Renderable2D *self = new CR::Gfx::Renderable2D(); // layer's flush is in charge of deleting this
...
CR::Gfx::Renderable *CR::Gfx::Draw::Texture(const std::shared_ptr<CR::Gfx::Texture> &tex, const CR::Vec2<float> &pos, const CR::Vec2<int> &size, const CR::Vec2<float> &origin, float angle){
    return CR::Gfx::Draw::Texture(*tex.get(), pos, size, origin, angle);
}
...
void CR::Gfx::render(){
    ...
    // We should probably move this later on
    uiL->clear();
    uiL->renderOn([](CR::Gfx::RenderLayer *layer){   
...
void CR::Gfx::onEnd(){
    ...
    for(unsigned i = 0; i < shListCopy.size(); ++i){
        CR::Gfx::deleteShader(texListCopy[i]); // TODO: Fix: GL_INVALID_OPERATION See: https://docs.gl/gl2/glDeleteShader
    }
